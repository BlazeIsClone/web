...
metaTitle: "Distributed Systems: CAP, PACELC, 2PC and the Saga Pattern"
publishedAt: "2026-05-02"
summary: "Field notes on distributed architecture: the CAP and PACELC theorems, two-phase commit versus the Saga pattern, and the trade-offs behind each choice."
---
...
A concrete example of this tradeoff is the [microservice auth with JWTs](/logs/jwt-microservice-auth) where an API Gateway acts as a trust boundary and validation checkpoint.
...
The [AWS high availability architecture](/logs/aws-cloud) applies these tradeoffs in practice, while [local-first PWAs](/logs/local-first-pwa) show how cache invalidation and consistency decisions shape client-side architecture.
