...
metaTitle: "Local-First PWA: IndexedDB and SSE for Instant Filtering"
publishedAt: "2026-06-25"
updatedAt: "2026-08-03"
summary: "Designing a dealership inventory catalog as a local-first PWA: load once into IndexedDB, filter in memory, and let Server-Sent Events push only what changed."
---
...
That model is reasonable for many public websites. It is less reasonable for a sales tool used throughout the day on shared showroom iPads. The challenge mirrors what [API auth boundaries](/logs/jwt-microservice-auth) face: deciding what the client can trust locally versus what requires a server roundtrip.
...
That is why the catalog is built around a local first browsing experience. Load the inventory once. Keep it on the device. Filter it locally. Cache as much as possible. Use a server signal to refresh when something changes. The cache-invalidation and consistency tradeoffs here echo the [distributed systems theory](/logs/distributed-architecture) behind CAP and PACELC.

The [hive bug tracker](/logs/hive-bug-tracker) uses the same TanStack ecosystem to solve realtime sync in a different domain.
