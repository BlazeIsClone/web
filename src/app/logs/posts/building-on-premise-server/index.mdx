...
import serverOpencase from "./server-opencase.webp";
import proxmox from "./proxmox.webp";
import argoTunnel from "./argo-tunnel.webp";
...
Self-hosting as a bare-metal homelab felt like a better path than going all-in on [AWS or the cloud](/logs/aws-cloud).
...
<PostImage src={serverOpencase} alt="server open case" priority />
...
<PostImage src={proxmox} alt="proxmox dashboard" />
...
<PostImage src={argoTunnel} alt="argo tunnel diagram" />
...
This manual setup contrasts with the [hardened VPS config](/logs/ansible-cloud-config) where all of this gets automated.
...
Once deployed, the [monitoring system](/logs/monitoring-system) ensures the server stays healthy and visible when issues arise.
